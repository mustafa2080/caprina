...
export default function Inventory() {
  ...
  const isAutoGeneratedSku = (variant: ProductVariant) => {
    const sku = (variant.sku ?? "").trim();
    const productName = (variant.productName ?? "").trim();
    if (!sku || !productName || !variant.color || !variant.size) return false;
    const expected = `${productName.substring(0, 3).toUpperCase()}-${variant.color.substring(0, 3).toUpperCase()}-${variant.size.toUpperCase()}`;
    return sku === expected;
  };
  const totalVariants = visibleVariants.filter(v => (v.sku ?? "").trim().length > 0 && !isAutoGeneratedSku(v)).length;
  const lowStockVariants = visibleVariants.filter(v => (v.sku ?? "").trim().length > 0 && !isAutoGeneratedSku(v) && v.totalQuantity > 0 && v.totalQuantity <= v.lowStockThreshold).length;
