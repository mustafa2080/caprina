  ...
  const handlePrint = async (invoiceNumbers = selectedIds) => {
    ...
    const styles = `
      ...
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .inv-body { padding: 1mm 3mm 0.5mm; flex: 1; overflow: hidden; min-height: 0; }
    ...
    const invoiceHTML = (grp: InvoiceGroup) => {
      ...
      const makeProductRows = (extraEmptyRows: number = 0) => {
        const rows = realOrders.map((o: any) => {
          const color = o.color ?? "";
          const size = o.size ?? "";
        ...
        for (let i = 0; i < extraEmptyRows; i++) rows.push(`<tr style="height:7.5mm"><td colspan="6" style="border:1px solid #ebebeb;background:#fafafa"></td></tr>`);
        return rows.join("");
      };
      ...
      return { html: (extraEmptyRows: number = 0) => `<div class="inv"><div class="inv-hdr" style="padding:${hdrPad}"><div class="hdr-logo"><div style="width:${logoSize};height:${logoSize};flex-shrink:0">${logoB64 ? `<img src="${logoB64}" style="width:100%;height:100%;object-fit:contain;border-radius:1.5mm;background:white;padding:0.5mm;" alt="${brandName}" />` : `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.1);border-radius:1.5mm;font-size:7pt;font-weight:900;color:white;">${brandName.substring(0,2)}</div>`}</div><div style="text-align:left;line-height:1.2;margin-right:2mm"><div class="logo-txt" style="font-size:11pt">${brandName}</div><div class="logo-sub">${brandTagline}</div></div></div><div class="hdr-date" style="font-size:8pt">${dateStr}<br/><span style="font-size:5pt;opacity:0.5">ORDER #${orderNum}</span></div></div><div class="cust-row" style="padding:${custPad}"><div class="cust-name" style="font-size:12pt">${grp.customerName}</div><div class="cust-phone" style="font-size:10pt">&#128222; ${grp.phone ?? "&#8212;"}</div></div><div class="inv-body"><div class="table-wrap"><table class="prod-table" style="font-size:${tblFontSize}pt"><thead><tr><th style="width:30%;padding:${cellPad}">الصنف</th><th style="width:14%;padding:${cellPad}">المقاس</th><th style="width:18%;padding:${cellPad}">اللون</th><th style="width:10%;padding:${cellPad}">العدد</th><th style="width:14%;padding:${cellPad}">السعر</th><th style="width:14%;padding:${cellPad}">الإجمالي</th></tr></thead><tbody>${makeProductRows(extraEmptyRows)}${shippingCost > 0 ? `<tr><td class="name-col" colspan="4" style="color:#777;font-size:${(parseFloat(tblFontSize)*0.85).toFixed(1)}pt;padding:${cellPad}">مصاريف الشحن</td><td colspan="2" style="font-weight:700;padding:${cellPad}">${formatCurrency(shippingCost)}</td></tr>` : ""}</tbody></table></div><div style="flex-shrink:0;display:flex;justify-content:space-between;align-items:center;background:#e0e0e0;border:1px solid #888;border-radius:1mm;padding:${cellPad};font-size:${tblFontSize}pt;font-weight:900;color:#000;margin-bottom:0.5mm"><span>&#9679; الإجمالي الكلي</span><span>${totalQty} قطعة &nbsp;|&nbsp; ${formatCurrency(totalPrice + shippingCost)}</span></div></div><div class="inv-mid-spacer"></div><div class="inv-bottom" style="font-size:${bottomFontSize}pt"><div class="info-strip"><div class="info-cell" style="padding:${bottomPad}"><span class="info-lbl">المحافظة</span><span class="info-val">${city || "&#8212;"}</span></div><div class="info-cell" style="padding:${bottomPad}"><span class="info-lbl">شركة الشحن</span><span class="info-val">${company ? company.name : "&#8212;"}</span></div><div class="info-cell" style="padding:${bottomPad}"><span class="info-lbl">رقم التتبع</span><span class="info-val" style="direction:ltr;text-align:right">${trackingNumber || "&#8212;"}</span></div></div><div class="addr-box" style="padding:${bottomPad}"><div class="addr-lbl">العنوان بالتفصيل</div><div class="addr-val">${address || "&#8212;"}</div></div><div class="notes-box" style="padding:${bottomPad};font-size:${bottomFontSize}pt"><b>&#128203; ملاحظات:</b><span>${notes || "&#8212;"}</span></div><div class="confirm-box" style="padding:${bottomPad};font-size:${bottomFontSize}pt"><span class="cb-lbl">&#10003; التاكيد علي الشحن:</span><span>تم التاكيد مع العميل &#8212; في حاله عدم الاستلام بيتم دفع مصاريف الشحن كامله المتفق عليها</span></div></div><div class="inv-footer"><div class="policy-txt">الاسترجاع فقط اثناء تواجد المندوب &middot; الاستبدال خلال 7 أيام &middot; ضمان 6 أشهر &middot; احتفظ بالفاتورة</div><div class="footer-brand">${brandName}</div></div></div>`, rowCount };
    };
