...
- **Shipping Company Detail Page** (`/shipping/company/:id`): Timeline of all manifests for a company — open manifests first (highlighted), closed below. Stats: total orders, delivered, returned, net profit. **Auto-rollover on close**: when a manifest is closed, pending/postponed orders automatically roll over to a new manifest. **Print-friendly manifest**: single A4 portrait with header, stats row, full orders table, footer with totals + signatures.
