...
import { Archive, RotateCcw, Building2, Star, RefreshCw } from "lucide-react";
    ...
    <div className="space-y-5 animate-in fade-in duration-500" dir="rtl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">الخزن المؤرشفة — يمكنك استعادتها في أي وقت</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-60 gap-4 text-muted-foreground rounded-2xl border border-border/40 bg-muted/10">
          ...
          <div className="text-center">
            ...
            <p className="text-xs mt-1 opacity-60">الخزن المحذوفة ستظهر هنا ويمكنك استعادتها</p>
          </div>
        </div>
      ) : (
          ...
          {data.registers.map(r => (
            <div key={r.id}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-muted-foreground/50">
                      أُرشف {r.archivedAt
                        ? new Date(r.archivedAt).toLocaleDateString("ar-EG")
                        : "—"}
                    </span>
                  </div>
                </div>
              ...
              <Button
                size="sm"
                variant="outline"
                className="w-full gap-1.5 text-xs h-8 rounded-xl border-emerald-300/50 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors"
