...
export default function Inventory() {
  ...
  const totalVariants = allVariants?.filter(v => (v.sku ?? "").trim().length > 0 && !isAutoGeneratedSku(v)).length ?? 0;
  const lowStockVariants = allVariants?.filter(v => (v.sku ?? "").trim().length > 0 && !isAutoGeneratedSku(v) && v.totalQuantity > 0 && v.totalQuantity <= v.lowStockThreshold).length ?? 0;
