    ...
    <div className={`border-b border-border/50 transition-colors ${editing ? "bg-primary/5" : "hover:bg-muted/10"}`}>
      ...
      {editing && (
        <div className="px-4 pb-3 flex flex-col gap-2 bg-primary/5 border-t border-primary/10">
          ...
          <div>
            <Label className="text-[10px] mb-1 block text-muted-foreground">
              {status === "returned" && returnReason === "other"
                ? "اكتب السبب الآخر هنا (مطلوب)"
                : needsNote ? "سبب / ملاحظة (مطلوب)" : "ملاحظة (اختياري)"}
            </Label>
            <Input
              value={note}
              onChange={(e) => setNote(e.target.value)}
              className={`h-8 text-xs bg-background ${status === "returned" && returnReason === "other" ? "border-amber-600/60 focus:ring-amber-600" : ""}`}
              placeholder={
                status === "returned" && returnReason === "other"
                  ? "اكتب تفاصيل السبب هنا..."
                  : status === "postponed"
  ...
  return (
    ...
    <div className="manifest-print print:block hidden" dir="rtl">
      ...
      <div className="mp-stats">
        ...
        <div className="mp-stat mp-stat-delivered"><div className="mp-stat-lbl">مسلَّم</div><div className="mp-stat-val">{s.delivered + groupedPartialCount}</div></div>
    ...
    <div className="manifest-screen print:hidden max-w-5xl mx-auto space-y-5 animate-in fade-in duration-500" dir="rtl">
      ...
      {(() => {
        ...
        // استلام جزئي — يظهر في الحاوية لو في كمية باقية عند الشحن (qty - partialQuantity > 0) ولم يتأكد استلامها
        const partialRemainderRows = manifest.orders.filter(o => {
          if (o.deliveryStatus !== "partial_received") return false;
          if (isReturnConfirmed(o)) return false;
          const remaining = (o.quantity ?? 0) - (o.partialQuantity ?? 0);
          return remaining > 0;
        });
